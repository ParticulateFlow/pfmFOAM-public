...
:
    ...
    deviationPropagators_
    (
        deviationPropagators::New
        (
            dataBaseProperties_,
            *this
        )
...
void dataBase::init()
{
    ...
    // read deviation propagators
    numRefStates_ = deviationPropagators_->readSenderIDs(dataBaseNames_);
    if (numRefStates != numRefStates_ && referenceStates_.valid())
    {
        FatalError << "different number of reference states and accompanying deviation propagator\n" << abort(FatalError);
    }
    deviationPropagators_->readDeviationPropagators(dataBaseNames_);
}
