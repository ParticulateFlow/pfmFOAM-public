...
#   include "createMesh.H"
    ...
    forAll (fields, fieldI)
    {
        ...
        (
            ...
            mesh,
            dimensionedScalar("zero", fields[fieldI].dimensions(), 0.0)
        );
        ...
        scalar sumFieldError =
            ...
            (
                sumSqr
                (
                    pReconstruct.primitiveField()
                  - fields[fieldI].primitiveField()
                )
            );

        scalar measure =
            Foam::sqrt(sumSqr(fields[fieldI].primitiveField())) + SMALL;
