...
Foam::BlendedInterfacialModel<modelType>::KdUdrift()
{
    ...
    (
        ...
        (
            ...
            pair_.phase1().mesh(),
            dimensionedVector("zero", modelType::dimF, vector(0.0,0.0,0.0))
         )
     );
    if (model_.valid())
    {
        x.ref() += model_->KdUdrift()*(f1() - f2());
    }
    if (model1In2_.valid())
    {
        x.ref() += model1In2_->KdUdrift()*(1 - f1);
    }
    if (model2In1_.valid())
    {
        x.ref() += model2In1_->KdUdrift()*f2;
    }
    if
        (
         correctFixedFluxBCs_
