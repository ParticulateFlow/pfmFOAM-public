...
//        Cp_     = CpScalar_;
    ...
    if (!equilibrium_) {
        volTensorField R2 = (k_&eX)*(eX*eX) + (k_&eY)*(eY*eY) + (k_&eZ)*(eZ*eZ)
                            - nut_*dev(gradU + gradU.T());
        volTensorField gradUR2 = 0.5*((R2&gradU) + ((R2.T())&(gradU.T())));
        volVectorField shearProd =   (gradUR2&&(eX*eX))*(eX)
                                   + (gradUR2&&(eY*eY))*(eY)
                                   + (gradUR2&&(eZ*eZ))*(eZ);
        /*
        volVectorField shearProd = -lm_*(
